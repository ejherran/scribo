...
class HomeController extends Controller
{
    ...
    public function facturAction($id)
    {
        ...
        {
            return $this->render('ScriboBundle:Home:visor.html.twig', array('URL' => $this->generateUrl('scribo_home_recibo', array('id' => $id))));
        }
        else
        {
            $this->get('session')->getFlashBag()->add('notice', 'Intento de acceso no autorizado!...');
    ...
    public function reciboAction($id)
    {
        if(Gestion::isGrant($this, 'F'))
        {
            $obj = new Repo();
            ...
            $pdf->SetCreator(PDF_CREATOR);
            $pdf->SetAuthor('IT Sribo');
            $pdf->SetTitle('Combrobante De Orden');
            ...
            $pdf->setImageScale(PDF_IMAGE_SCALE_RATIO);
            $pdf->setTabl(true);
            $pdf->setMemoTitle("COMPROBANTE DE ORDEN: ".$id);
